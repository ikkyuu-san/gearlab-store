...
export default async function AdminProductsPage({ searchParams }: PageProps<"/admin/products">) {
  ...
  return (
    <AdminShell>
      ...
      {products.length === 0 ? <div className="admin-panel admin-empty"><h2>No products yet.</h2><p>Create the first product to start building the catalog.</p><Link className="button button-secondary" href="/admin/products/new">Create product</Link></div> : <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Product</th><th>Category</th><th>Price</th><th>Stock</th><th>Featured</th><th><span className="sr-only">Actions</span></th></tr></thead><tbody>{products.map((product) => <tr key={product.id}><td><strong>{product.name}</strong><small>{product.slug}</small></td><td>{product.category}</td><td>{product.priceTHB.toLocaleString()} THB</td><td><span className={`admin-status admin-status-${product.stockStatus.toLowerCase()}`}>{statusLabels[product.stockStatus]}</span></td><td>{product.featured ? "Yes" : "No"}</td><td><div className="admin-table-actions"><Link className="admin-table-action" href={`/admin/products/${product.id}/edit`}>Edit</Link><DeleteProductButton id={product.id} name={product.name} /></div></td></tr>)}</tbody></table></div>}
    </AdminShell>
  );
}
